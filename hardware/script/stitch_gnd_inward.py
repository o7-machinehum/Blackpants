...
cuts = [d for d in board.GetDrawings() 
        if d.GetShape() == pcbnew.SHAPE_T_SEGMENT and d.GetLayer() == pcbnew.Edge_Cuts]
...
def stitch_segment(p1, p2):
    ...
    n1 = (-uy, ux)
    n2 = (uy, -ux)
    # Pick inward normal: test midpoint offset in each direction
    mx = (p1.x + p2.x) / 2
    my = (p1.y + p2.y) / 2
    # Test both
